...
// This intentionally overrides only Andrew/F01 so the corrected sheet structure can be validated
// before the remaining team tabs are normalized and imported.
(function () {
  const TEST_FID = 'F01';
  ...
  function currentRouteIsTest() {
    const parts = location.hash.replace(/^#/,'').split('/');
    const currentSeason = Number(typeof season !== 'undefined' ? season : document.getElementById('seasonSelect')?.value);
    return parts[0] === 'team' && parts[1] === TEST_FID && (!parts[2] || parts[2] === 'overview') && currentSeason === TEST_SEASON;
  }

  function playerIdFor(name) {
    if (!name || typeof DB === 'undefined' || !DB?.Players) return '';
    const target = norm(name);
    return DB.Players.find(p => norm(p.Player_Name) === target)?.Player_ID || '';
  }

  function playerCell(row) {
    if (!row.name) return '<span class="andrew-empty-slot">Available G-League slot</span>';
    const pid = playerIdFor(row.name);
    const name = pid ? `<span class="player-link" onclick="go('player/${pid}')">${escLocal(row.name)}</span>` : escLocal(row.name);
    return `${name}${row.note ? `<small class="contract-status-note">${escLocal(row.note)}</small>` : ''}`;
  }
  ...
  function applyAndrewTest() {
    ...
    table.innerHTML = `
      ...
      <tbody>${MAIN_ROSTER.map(row => `<tr class="contract-roster-row${row.status === 'TW' ? ' two-way-cap-exempt' : ''}"><td class="contract-player-col">${playerCell(row)}</td>${unitCells(row.units)}</tr>`).join('')}</tbody>
      <tfoot><tr class="contract-grid-total"><th>CAP TOTAL</th>${CAP_TOTALS.map(v => `<th class="contract-year-total">${v}</th>`).join('')}</tr></tfoot>`;

    document.querySelector('.g-league-reserves-card')?.remove();
    const section = document.createElement('section');
    section.className = 'card team-panel g-league-reserves-card andrew-g-league-test';
    section.innerHTML = `
      <div class="card-pad section-title"><div><div class="eyebrow">DEVELOPMENT ROSTER</div><h2>G-League</h2></div><span>5 roster slots · does not count toward cap</span></div>
      <div class="table-wrap"><table class="data-table contract-year-grid g-league-table">
        <thead><tr><th class="contract-player-col">Player</th>${YEARS.map(y => `<th class="contract-year-head">${y}</th>`).join('')}</tr></thead>
        <tbody>${G_LEAGUE.map(row => `<tr><td class="contract-player-col">${playerCell(row)}</td>${unitCells(row.units)}</tr>`).join('')}</tbody>
      </table></div>`;
  ...
  style.textContent = `
    .andrew-cap-test-grid .two-way-cap-exempt td{opacity:.82}
    .andrew-g-league-test .contract-year-grid{min-width:720px}
    .andrew-empty-slot{opacity:.35;font-style:italic}
  `;
