...
(function(){
  ...
  function renderLeague(){
    ...
    const byOwner=new Map(owners.map(x=>[x.owner,x]));
    const standings=live.standings||[];
    const draft=live.draft||[];
    const slotOrder=live.slotOrder||[];

    app.innerHTML=`<section class="page-head"><div class="eyebrow">Dynasty NBA · ${live.displaySeason}</div><h1>Wins Pool</h1><p>Current-year Wins Pool standings and selection board from the live master Google Sheet.</p></section><div class="content">
      <section class="card"><div class="card-pad section-title"><h2>${live.displaySeason} WINS POOL STANDINGS</h2><span>Highest total earns the first base free-agency position</span></div><div class="table-wrap"><table class="data-table"><thead><tr><th>#</th><th>Owner</th><th>Total</th><th>Projected</th><th>Preseason Total</th><th>Vs. Preseason</th><th>Games Remaining</th></tr></thead><tbody>${standings.map(x=>`<tr><td class="rank">${x.position}</td><td><b>${esc(x.owner)}</b></td><td class="record">${num(x.total)}</td><td>${num(x.projected)}</td><td>${num(x.preseason)}</td><td>${signed(x.projected==null||x.preseason==null?null:x.projected-x.preseason)}</td><td>${num(x.remaining)}</td></tr>`).join('')}</tbody></table></div></section>
      <section class="card"><div class="card-pad section-title"><h2>OWNER SELECTIONS</h2><span>Three NBA teams per owner</span></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Owner</th><th>Pick</th><th>NBA Team</th><th>W-L</th><th>Projected Wins</th><th>Preseason O/U</th><th>Diff</th></tr></thead><tbody>${owners.map(x=>(x.teams||[]).map((t,i)=>`<tr>${i===0?`<td rowspan="${x.teams.length}"><b>${esc(x.owner)}</b><br><small>${num(x.total)} total wins</small></td>`:''}<td>${num(t.pick)}</td><td><b>${esc(t.team)}</b></td><td class="record">${num(t.wins)}-${num(t.losses)}</td><td>${num(t.projected)}</td><td>${num(t.preseasonOU)}</td><td>${signed(t.diff)}</td></tr>`).join('')).join('')}</tbody></table></div></section>
