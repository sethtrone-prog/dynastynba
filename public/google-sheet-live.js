...
(function(){
  ...
  function renderCap(){
    ...
    table.innerHTML=`<colgroup><col style="width:34%">${years.map(()=>'<col style="width:13.2%">').join('')}</colgroup><thead><tr><th class="contract-player-col">Player</th>${years.map(y=>`<th class="contract-year-head">${esc(y)}</th>`).join('')}</tr></thead><tbody>${numberedRows.map(({row,marker})=>`<tr class="${row.slot==='TW'?'two-way-cap-exempt':String(row.slot||'').trim().toLowerCase()==='dead cap'?'dead-cap-row':''}"><td class="contract-player-col">${playerCell(row,marker)}</td>${unitCells(row.units,years)}</tr>`).join('')}<tr class="team-total-row"><td class="contract-player-col" style="color:#f2c55d"><strong style="color:#f2c55d">TEAM TOTAL</strong></td>${(data.totals||[]).map(v=>`<td class="contract-year-total" style="text-align:center;color:#f2c55d"><strong style="color:#f2c55d">${v??0}</strong></td>`).join('')}</tr></tbody>`;
