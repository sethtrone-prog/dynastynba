...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows=(data.players||[]).map(entry=>{const p=entry.player||{}; const stat=(p.stats||[]).find(s=>Number(s.seasonId)===Number(season)&&Number(s.statSourceId)===0&&Number(s.statSplitTypeId)===0); const seasons=(p.stats||[]).filter(s=>Number(s.statSourceId)===0&&Number(s.statSplitTypeId)===0&&Number(s.seasonId)>0).map(s=>Number(s.seasonId)); const firstSeason=seasons.length?Math.min(...seasons):null; return {player:p.fullName||null,espnId:p.id??entry.id??null,proTeamId:p.proTeamId??null,gp:stat?.stats?.['42']??null,fp:stat?.appliedTotal??null,ppg:stat?.appliedAverage??null,entryYear:firstSeason?firstSeason-1:null,draftYear:p.draftYear??null,experience:p.experience??null,rookieYear:p.rookieYear??null};}).filter(r=>r.gp!==null).sort((a,b)=>(b.ppg??-999)-(a.ppg??-999));
    ...
    if(String(req.query.compact||'')==='1') { const teams=(data.teams||[]).map(t=>({espnTeamId:t.id??null,logo:t.logo||null,name:t.name||[t.location,t.nickname].filter(Boolean).join(' ')||null})); return res.status(200).json({season:Number(season),validCount:rows.length,players:rows,teams}); }
