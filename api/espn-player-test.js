...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows=(data.players||[]).map(entry=>{const p=entry.player||{}; const stat=(p.stats||[]).find(s=>Number(s.seasonId)===Number(season)&&Number(s.statSourceId)===0&&Number(s.statSplitTypeId)===0); return {player:p.fullName||null,espnId:p.id??entry.id??null,gp:stat?.stats?.['42']??null,fp:stat?.appliedTotal??null,ppg:stat?.appliedAverage??null};}).filter(r=>r.gp!==null).sort((a,b)=>(b.ppg??-999)-(a.ppg??-999));
